...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
                ...
                when(event){
                    SaveSamples->{
                        val intent=Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                            ...
                            type="text/plain"
                            putExtra(Intent.EXTRA_TITLE, viewModel.getFileName())
                        }
                        startActivityForResult(intent,REQUEST_CREATE_FILE)
                    }
                    else->super.onActionEvent(event)
                }
            }
                ...
                if(requestCode==REQUEST_CREATE_FILE){
                    when(requestCode){
                        Activity.RESULT_OK->{
                            ...
                            viewModel.saveSamples(outputStream)
                        }
                    }
                }
            }
        ...
        ppiButton.setOnClickListener {
            ...
            val stream = applicationContext.contentResolver.openOutputStream(dstUri!!)
            val unixTime = System.currentTimeMillis()
            stream?.write("Start time = ${unixTime};".toByteArray())
            stream?.write("\n".toByteArray())
            stream?.write("Timestamp;PPI".toByteArray())
            stream?.write("\n".toByteArray())
                ...
                ppiDisposable = api.startOhrPPIStreaming(deviceId)
                    ...
                    .subscribe(
                        { ppiData: PolarOhrPPIData ->
                            ...
                            //val path: File = Environment.getExternalStoragePublicDirectory("Android/data")
                            //val file = File(path, filename)//узнать нужна ли эта строска, возможно без нее перезаписывается текст
                            val unixTime = System.currentTimeMillis()//посмотреть другие функции для вывода timestamp
                            try {
                                //val stream = applicationContext.contentResolver.openOutputStream(dstUri!!)
                                //val numbers =   ppiData.samples
                                // val bytes = polarExerciseData.hrSamples.foldIndexed(ByteArray(polarExerciseData.hrSamples.size)) { i, a, v -> a.apply { set(i, v.toByte()) } }
                                //stream.write("[".toByteArray())
                                //val mycal1=LocalTime.now()
                                //val formatter = SimpleDateFormat("hh:mm:ss")
                                //val time = formatter.format(Date())
                                //val mycal1 = Calendar.getInstance()
                                //Long unixTime = System.currentTimeMillis() / 1000L
                                //val unixTime = System.currentTimeMillis()//посмотреть другие функции для вывода timestamp
                                for (sample in ppiData.samples) {
                                    //stream.write("${mycal1.getTime()} ".toByteArray())
                                    //stream.write("${time} ".toByteArray())
                                    stream?.write("${unixTime};".toByteArray())
                                    stream?.write(("${sample.ppi}").toByteArray())
                                    stream?.write("\n".toByteArray())
                                }
                                    //stream?.close()
                            ...
                            for (sample in ppiData.samples) {
                                Log.d(TAG, "PPI    ppi: ${sample.ppi} blocker: ${sample.blockerBit} errorEstimate: ${sample.errorEstimate} heartRate: ${sample.hr}")
                                //Log.d(TAG, "Unix timestamp: ${unixTime}")
                                println("Unix timestamp: ${unixTime}")
                            }
                        },
                        { error: Throwable ->
